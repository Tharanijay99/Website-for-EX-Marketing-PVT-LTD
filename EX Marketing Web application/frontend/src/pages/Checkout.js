...
const Checkout = () => {
  ...
  const applyCoupon = (e) => {
    ...
    // Get and apply the coupon
    axios.get(`${base_url}coupon/getByName/${coupon}`)
      .then((res) => {
        const c = res.data.discount;
        setCartPrice(cartPrice * ((100 - c) / 100));
        setAppliedCoupon(coupon); // Store the applied coupon name
        toast.success("YOUR COUPON APPLIED");
      })
      .catch((e) => {
        toast.error("YOUR COUPON IS INVALID");
      });
  }
